  ...
  struct MPSCLockFreeQueue //: private boost::noncopyable
  {
    ...
    Block Reserve(const std::size_t num)
    {
      ...
      do
      {
        ...
        const auto current_reader_pos = reader_tail_.load(std::memory_order_acquire);
        if (current_reader_pos + capacity_ >= prev_writer_head)
        {
          return res; // we are full
        }
        const auto max_available_elems = prev_writer_head - capacity_ - current_reader_pos;
        num_items_to_reserve = std::min(max_available_elems, num);
      } while (!writer_head_.compare_exchange_weak(prev_writer_head, new_writer_head, std::memory_order_release, std::memory_order_relaxed));
      res.seq_no_ = prev_writer_head + 1;
    ...
    Block Read(std::size_t n, ProcFunc &&process_func)
    {
      assert(n > 0);
      ...
      reader_tail_.store(items_to_read);
    }
